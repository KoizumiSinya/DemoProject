package com.sap360.demo_springlistviewscrollview.widget;
...
public class SpringListView extends ListView {
    ...
    GestureDetector gestureDetector = new GestureDetector(new OnGestureListener() {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            ...
            if (!outBound) firstOut = (int) e2.getRawY();
            ...
            if (lastPos != (itemCount - 1)) return false;
            View lastView = getChildAt(lastPos - firstPos);
            int GridHeight = getHeight();
            if (lastView != null && (outBound || ((lastView.getBottom() + 8) >= GridHeight && distanceY > 0))) {
                distance = firstOut - (int) e2.getRawY();
                scrollTo(0, distance);
                return true;
            }
