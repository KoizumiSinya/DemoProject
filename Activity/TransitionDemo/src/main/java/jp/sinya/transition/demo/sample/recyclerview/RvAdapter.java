...
public class RvAdapter extends CommonAdapter<String> {
    ...
    protected void convert(final ViewHolder holder, final String str, int position) {
        Glide.with(mContext).load(str).into((ImageView) holder.itemView.findViewById(R.id.iv1));
        holder.setOnClickListener(R.id.iv1, new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                TransitionsHelper.startActivity((Activity) mContext, RvDetailActivity.class, holder.itemView.findViewById(R.id.iv1), str);
            }
        });
    }
}
