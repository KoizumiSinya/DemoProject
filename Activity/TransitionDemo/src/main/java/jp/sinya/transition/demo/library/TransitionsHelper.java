package jp.sinya.transition.demo.library;
...
import jp.sinya.transition.demo.library.expose.CircleExposeView;
import jp.sinya.transition.demo.library.expose.base.ExposeView;
import jp.sinya.transition.demo.library.listener.ExposeListener;
import jp.sinya.transition.demo.library.listener.TransitionListener;
...
import jp.sinya.transition.demo.library.method.ShowMethod;

import static jp.sinya.transition.demo.library.util.BitmapUtils.createBitmap;
...
public class TransitionsHelper {
    ...
    private static HashMap<String, WeakReference<TransitionsHelper>> sTransitionMap = new HashMap<>();
    ...
    private TransitionsHelper(TransitionBuilder builder) {
        ...
        sTransitionMap.put(activity.getClass().getName(), new WeakReference<>(this));
    }
    ...
    private static TransitionsHelper create(TransitionBuilder builder) {
        return new TransitionsHelper(builder);
    }
    ...
    private static void startEngine(final Activity activity, final Class<?> cls, //
                                    Intent intent, final View view, //
                                    final Object load, final boolean isForResult,//
    ...
    private void show() {
        ...
        final ViewGroup parent = activity.findViewById(Window.ID_ANDROID_CONTENT);
        //if TranslucentStatus is true , statusBarHeight = 0
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            if (activity.getWindow().getStatusBarColor() == 0 || (WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS & activity.getWindow().getAttributes().flags) == WindowManager.LayoutParams
        ...
        Runnable r = new Runnable() {
            ...
            public void run() {
                ...
                if (exposeView == null) {
                    exposeView = new CircleExposeView(activity);
                }
                if (showMethod instanceof InflateShowMethod) {
                    exposeView.setInflateBitmap(createBitmap(((InflateShowMethod) showMethod).inflateView, bean.windowWidth, bean.windowHeight, true));
                }
                ...
                if (bean.originRect.top + bean.originHeight > (bean.windowHeight + bean.statusBarHeight + bean.titleHeight)) {
                    bean.scale = (float) (bean.windowHeight + bean.statusBarHeight + bean.titleHeight - bean.originRect.top) / bean.targetHeight;
                } else {
                    bean.scale = (float) bean.originHeight / bean.targetHeight;
                }

                RelativeLayout.LayoutParams ivTempParams = new RelativeLayout.LayoutParams((int) (bean.targetWidth * bean.scale), (int) (bean.targetHeight * bean.scale));
                ivTempParams.setMargins((int) (bean.originRect.left + (bean.originWidth / 2 - bean.targetWidth * bean.scale / 2)), bean.originRect.top - (parent.getTop() + bean.statusBarHeight), 0,
                        0);
    ...
    public static void unbind(Activity activity) {
        if (sTransitionMap.get(activity.getClass().getName()) != null) {
            WeakReference<TransitionsHelper> weakT = sTransitionMap.get(activity.getClass().getName());
    ...
    public static class TransitionBuilder {
        ...
        private int transitionDuration = ShowMethod.DEFAULT_DURATION;
        ...
        public void show() {
            TransitionsHelper t = TransitionsHelper.create(this);
            t.show();
        }
