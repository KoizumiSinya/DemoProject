...
package jp.sinya.seekbar.widget;
...
public class RangeSeekBar<T extends Number> extends ImageView {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs == null) {
            ...
            try {
                setRangeValues(extractNumericValueFromAttributes(a, R.styleable.RangeSeekBar_absoluteMinValue, DEFAULT_MINIMUM), extractNumericValueFromAttributes(a, R.styleable
                        .RangeSeekBar_absoluteMaxValue, DEFAULT_MAXIMUM));
        ...
        mDistanceToTop = PixelUtil.dpToPx(context, -10);
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int height = thumbImage.getHeight() + (!mShowTextAboveThumbs ? 0 : PixelUtil.dpToPx(getContext(), HEIGHT_IN_DP)) + (mThumbShadow ? mThumbShadowYOffset + mThumbShadowBlur : 0);
    ...
    protected synchronized void onDraw(@NonNull Canvas canvas) {
        ...
        paint.setColor(mDefaultColor);
        canvas.drawCircle(getWidth() - padding, mRect.centerY(), 5, paint);
        ...
        canvas.drawCircle(mRect.left, mRect.centerY(), 5, paint);
        ...
        canvas.drawText(maxText, normalizedToScreen(normalizedMaxValue) - maxTextWidth * 0.5f, 15 * PixelUtil.dpToPx(getContext(), DEFAULT_TEXT_DISTANCE_TO_TOP_IN_DP), paint);
        //}

    }

    /**
     * Overridden to save instance state when device orientation changes. This method is called automatically if you assign an id to the RangeSeekBar widget using the {@link #setId(int)} method.
    ...
    private Thumb evalPressedThumb(float touchX) {
        ...
        if (minThumbPressed && maxThumbPressed) {
            // if both thumbs are pressed (they lie on top of each other), choose the one with more room to drag. this avoids "stalling" the thumbs in a corner, not being able to drag them apart
            // anymore.
     ...
     */
    @SuppressWarnings("unchecked")
    public T normalizedToValue(double normalized) {
