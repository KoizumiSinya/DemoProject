...
public class CardScaleHelper {
    ...
    public CardScaleHelper(boolean isHorizontal) {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                if (dx != 0) {//去掉奇怪的内存疯涨问题
                    ...
                    if (isHorizontal) {
                        ...
                        //log.i(String.format("dx=%s, dy=%s, mScrolledX=%s", dx, dy, mCurrentItemOffset));
                        onScrolledChangedCallback();
                    } else {
                        mCurrentItemOffset += dy;
                        computeCurrentItemPos();
                        //log.i(String.format("dx=%s, dy=%s, mScrolledX=%s", dx, dy, mCurrentItemOffset));
                        onScrolledChangedCallback();
                    }
    ...
    private void initWidth() {
        mRecyclerView.post(new Runnable() {
            @Override
            public void run() {
                mCardGalleryWidth = isHorizontal ? mRecyclerView.getWidth() : mRecyclerView.getHeight();
