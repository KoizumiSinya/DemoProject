...
import jp.sinya.recyclerviewgallery.demo.lib2.transform.DiscreteScrollItemTransformer;
...
class DiscreteScrollLayoutManager extends RecyclerView.LayoutManager {
    ...
    protected void updateRecyclerDimensions(RecyclerView.State state) {
        boolean dimensionsChanged = !state.isMeasuring() && (recyclerViewProxy.getWidth() != viewWidth || recyclerViewProxy.getHeight() != viewHeight);
    ...
    protected void fill(RecyclerView.Recycler recycler) {
        ...
        final int endBound = orientationHelper.getViewEnd(recyclerViewProxy.getWidth(), recyclerViewProxy.getHeight());
    ...
    private void layoutViews(RecyclerView.Recycler recycler, Direction direction, int endBound) {
        ...
        boolean noPredictiveLayoutRequired = pendingPosition == NO_POSITION || !direction.sameAs(pendingPosition - currentPosition);
    ...
    protected void layoutView(RecyclerView.Recycler recycler, int position, Point viewCenter) {
        ...
        if (v == null) {
            ...
            recyclerViewProxy.layoutDecoratedWithMargins(v, viewCenter.x - childHalfWidth, viewCenter.y - childHalfHeight, viewCenter.x + childHalfWidth, viewCenter.y + childHalfHeight);
        } else {
    ...
    protected int calculateAllowedScrollIn(Direction direction) {
        ...
        if (direction == Direction.START && currentPosition == 0) {
            ...
            allowedScroll = isScrollDirectionAsBefore ? scrollToChangeCurrent - Math.abs(scrolled) : scrollToChangeCurrent + Math.abs(scrolled);
        }
    ...
    private float getCenterRelativePositionOf(View v, int maxDistance) {
        float distanceFromCenter = orientationHelper.getDistanceFromCenter(recyclerCenter, getDecoratedLeft(v) + childHalfWidth, getDecoratedTop(v) + childHalfHeight);
    ...
    private void notifyScroll() {
        float amountToScroll = pendingPosition != NO_POSITION ? Math.abs(scrolled + pendingScroll) : scrollToChangeCurrent;
