...
package jp.sinya.circlewheelview.wheelview;
...
import jp.sinya.circlewheelview.R;
import jp.sinya.circlewheelview.wheelview.adapter.WheelAdapter;
import jp.sinya.circlewheelview.wheelview.transformer.FadingSelectionTransformer;
import jp.sinya.circlewheelview.wheelview.transformer.ScalingItemTransformer;
import jp.sinya.circlewheelview.wheelview.transformer.WheelItemTransformer;
import jp.sinya.circlewheelview.wheelview.transformer.WheelSelectionTransformer;
...
public class WheelView extends View {
    ...
    public void setWheelRadius(int radius) {
        if (radius < -1)
            throw new IllegalArgumentException("Invalid Wheel Radius: " + radius);
    ...
    private void setWheelBounds(int width, int height) {
        ...
        if (isPositionLeft())
            relativeHorizontal -= 0.5f;
        if (isPositionRight())
            relativeHorizontal += 0.5f;
        if (isPositionTop())
            relativeVertical -= 0.5f;
        if (isPositionBottom())
            relativeVertical += 0.5f;
    ...
    private int measureWheelRadius(int radius, int width, int height) {
        if (radius == ViewGroup.LayoutParams.MATCH_PARENT) {
            return Math.min(width - getPaddingLeft() - getPaddingRight(), height - getPaddingTop() - getPaddingBottom()) / 2;
        } else {
            return radius;
        }
    }
    ...
    private void setSelectedPosition(int position) {
        if (mRawSelectedPosition == position)
            return;
    ...
    public boolean onTouchEvent(@NonNull MotionEvent event) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_UP:
                if (mOnItemClickListener != null && mClickedItem != null && mClickedItem == getClickedItem(x, y) && mDraggedAngle < CLICK_MAX_DRAGGED_ANGLE) {
                    boolean isSelected = Math.abs(mClickedItem.mRelativePos) < 1f;
                    mOnItemClickListener.onWheelItemClick(this, mClickedItem.mAdapterPosition, isSelected);
                }
    ...
    private void flingWheel() {
        ...
        if (angularVel > MAX_ANGULAR_VEL)
            angularVel = MAX_ANGULAR_VEL;
        else if (angularVel < -MAX_ANGULAR_VEL)
            angularVel = -MAX_ANGULAR_VEL;
        ...
        for (int i = offset; i < length; i++) {
            ...
            if (Rect.intersects(sTempRect, mViewBounds)) {
                ...
                if (i == mRawSelectedPosition && mSelectionDrawable != null && !isEmptyItemPosition(i)) {
                    mSelectionDrawable.setBounds(sTempRect.left - mSelectionPadding, sTempRect.top - mSelectionPadding, sTempRect.right + mSelectionPadding, sTempRect.bottom + mSelectionPadding);
    ...
    private ItemState getClickedItem(float touchX, float touchY) {
        for (ItemState state : mItemStates) {
            ...
            if (itemBounds.contains(touchX, touchY))
                return state;
        }
        return null;
    }
