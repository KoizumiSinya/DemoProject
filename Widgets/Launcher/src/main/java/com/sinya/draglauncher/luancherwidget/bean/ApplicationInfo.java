...
public class ApplicationInfo extends ItemInfo {
     ...
     * 绘图标和文字
     */
    public void drawBoundIcon(LayoutCalculator lc, ObjectPool op, Canvas canvas, int x, int y, Paint paint, Paint iconPaint) {
     ...
     * 绘文件夹外框
     */
    public void drawFolderBound(LayoutCalculator lc, ObjectPool pp, Canvas canvas, int x, int y, Paint paint, float scale) {
        if (isAboveFolder != -1) {
    ...
    public void drawTitle(LayoutCalculator lc, Paint paint, Canvas canvas, int x, int y) {
        ...
        if (!titleMeasured) measureTitle(paint, lc);
    ...
    public void measureTitle(Paint paint, LayoutCalculator lc) {
        ...
        float scale = 1;
        if (length > itemWidth) scale = length / text.length();
        for (int j = (text.length() - (int) Math.ceil((length - itemWidth) / scale)); paint.measureText(text, 0, text.length()) > itemWidth; --j) {
