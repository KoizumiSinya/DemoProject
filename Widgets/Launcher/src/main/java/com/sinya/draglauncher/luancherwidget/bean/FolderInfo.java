...
public class FolderInfo extends ApplicationInfo {
    private Vector<ApplicationInfo> apps = new Vector<>();//包含的应用
    ...
    public void drawIconContent(LayoutCalculator lc, ObjectPool op, Canvas canvas, int x, int y, Paint iconPaint) {
        ...
        for (int i = 0; i < apps.size() && i < 9; i++) {
            ...
            if (info != null) {
                ...
                if (bitmap != null) {
                    content.drawBitmap(bitmap, new Rect(0, 0, bitmap.getWidth(), bitmap.getHeight()), new RectF(x1, y1, x1 + folderIconSize, y1 + folderIconSize), paint);
                }
            }
        }
