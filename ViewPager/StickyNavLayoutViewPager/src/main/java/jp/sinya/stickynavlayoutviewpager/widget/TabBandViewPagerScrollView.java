package jp.sinya.stickynavlayoutviewpager.widget;
...
import jp.sinya.stickynavlayoutviewpager.R;

/**
 * 自定义控件 可滑动的Tab绑定ViewPager
 * ①ScrollView 封装TabItem 选项栏目，根据接收的title文本自动生成tab选项卡
 * ②需要接收一个ViewPager 并绑定监听
 * ③提供可扩展功能的接口，修改显示样式等
 * ④添加小圆点提醒控件
 * ⑤提供代码修改属性和xml布局设置属性的功能
 *
 * @author Sinya
 * @date 2015/11/18
 */
public class TabBandViewPagerScrollView extends HorizontalScrollView {
     ...
     * Item 背景颜色
     */
    private int tabBackgroundResId = R.drawable.selector_tabbandviewpagerscrollview_bg_tab;
    ...
    public TabBandViewPagerScrollView(Context context) {
        this(context, null);
    }

    public TabBandViewPagerScrollView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public TabBandViewPagerScrollView(Context context, AttributeSet attrs, int defStyle) {
        ...
        //取得attrs属性
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.TabBandViewPagerScrollView);

        //Tab文本大小
        tabTextSize = a.getDimensionPixelSize(R.styleable.TabBandViewPagerScrollView_tabPagerTextSize, tabTextSize);
        //Tab文本颜色
        tabTextColor = a.getColor(R.styleable.TabBandViewPagerScrollView_tabPagerTextColor, indicatorColor);
        //Tab文本选中时颜色
        selectedTabTextColor = a.getColor(R.styleable.TabBandViewPagerScrollView_tabPagerTextSelectedColor, indicatorColor);
        //Tab指示条颜色
        indicatorColor = a.getColor(R.styleable.TabBandViewPagerScrollView_tabPagerIndicatorColor, indicatorColor);
        //分隔线颜色
        underlineColor = a.getColor(R.styleable.TabBandViewPagerScrollView_tabPagerUnderlineColor, underlineColor);
        //Item之间的分隔线颜色
        dividerColor = a.getColor(R.styleable.TabBandViewPagerScrollView_tabPagerDividerColor, dividerColor);
        //指示条的高度
        indicatorHeight = a.getDimensionPixelSize(R.styleable.TabBandViewPagerScrollView_tabPagerIndicatorHeight, indicatorHeight);
        //分隔线的高度
        underlineHeight = a.getDimensionPixelSize(R.styleable.TabBandViewPagerScrollView_tabPagerUnderlineHeight, underlineHeight);
        //Item之间的分隔线和Item间距
        dividerPadding = a.getDimensionPixelSize(R.styleable.TabBandViewPagerScrollView_tabPagerDividerPadding, dividerPadding);
        //Item之间的间距
        tabPadding = a.getDimensionPixelSize(R.styleable.TabBandViewPagerScrollView_tabPagerItemPaddingLeftRight, tabPadding);
        //Item的背景颜色
        tabBackgroundResId = a.getResourceId(R.styleable.TabBandViewPagerScrollView_tabPagerItemBackground, tabBackgroundResId);
        //是否使用权重
        shouldExpand = a.getBoolean(R.styleable.TabBandViewPagerScrollView_tabPagerShouldExpand, shouldExpand);
        //移动偏移量
        scrollOffset = a.getDimensionPixelSize(R.styleable.TabBandViewPagerScrollView_tabPagerIndicatorScrollOffset, scrollOffset);
        //tab 的文字（字母）是否大写
        textAllCaps = a.getBoolean(R.styleable.TabBandViewPagerScrollView_tabPagerTextAllCaps, textAllCaps);

        //tab 样式
        tabStyle = a.getInt(R.styleable.TabBandViewPagerScrollView_tabPagerStyle, tabStyle);
        //Icon宽
        tabIconWidth = a.getDimensionPixelSize(R.styleable.TabBandViewPagerScrollView_tabPagerIconWidth, tabIconWidth);
        //Icon高
        tabIconHeight = a.getDimensionPixelSize(R.styleable.TabBandViewPagerScrollView_tabPagerIconHeight, tabIconHeight);
        //Icon 和 文本间距
        tabIconPadding = a.getDimensionPixelSize(R.styleable.TabBandViewPagerScrollView_tabPagerIconToTextPadding, tabIconPadding);
        //文本距离底下分隔线的间距
        tabTextPaddingButtom = a.getDimensionPixelSize(R.styleable.TabBandViewPagerScrollView_tabPagerTextPaddingButtom, tabTextPaddingButtom);
        //文本字体的样式 默认0正常； 1是粗体
        tabTextStyle = a.getInt(R.styleable.TabBandViewPagerScrollView_tabPagerTextStyle, tabTextStyle);
    ...
    private void addIconAboveTextItem(final int position, String title, int resId) {
        ...
        ImageView image = new ImageView(getContext());
        image.setLayoutParams(new LinearLayout.LayoutParams(tabIconWidth, tabIconHeight));
        image.setPadding(0, 8, 0, px2dip(getContext(), tabIconPadding));
        image.setImageResource(resId);
        layout.addView(image);
    ...
    private void setTabTextSelector(View v, int i) {
        if (v instanceof TextView) {
            ...
            if (i <= tabTips.length - 1) {
                ...
                badgeView.setBackground(360, getContext().getResources().getColor(R.color.red_tip_bg));
    ...
    private void setTabIconSelector(View v, int i) {
        if (v instanceof ImageButton) {
            ...
            if (i <= tabTips.length - 1) {
                ...
                badgeView.setBackground(360, getContext().getResources().getColor(R.color.red_tip_bg));
                badgeView.setBadgeMargin(px2dip(getContext(), image.getMeasuredWidth() / 2 + badgeView.getMeasuredWidth() / 6 - tabPadding), 0, 0, px2dip(getContext(), badgeView.getMeasuredHeight()
                        / 2));
