...
public class RainbowProgressBar extends View {
    ...
    public RainbowProgressBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray attributes = context.obtainStyledAttributes(attrs, R.styleable.RainbowProgressBar);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (mViewWidth == 0) {
            ...
            if (mViewWidth > 0) {
                firstPaint = getPaint();
                firstShader = new LinearGradient(0, 0, getMeasuredWidth(), getHeight(), colorsRainbow, null, Shader.TileMode.CLAMP);
                firstPaint.setShader(firstShader);
                firstMatrix = new Matrix();

                secondPaint = getPaint();
                secondShader = new LinearGradient(-getMeasuredWidth(), 0, 0, getHeight(), colorsRainbow, null, Shader.TileMode.CLAMP);
...
//        canvas.drawRect(new RectF(0, 0, mTranslate, getHeight()), secondPaint);
    ...
    private void drawLine(Canvas canvas) {
        ...
        shader = new LinearGradient(0, 0, measuredWidth, 0, colors, null, Shader.TileMode.MIRROR);
    ...
    private void drawCircle(Canvas canvas) {
        ...
        shader = new LinearGradient(x - radius, y - radius, x - radius + radius, y + radius, colors, null, Shader.TileMode.MIRROR);
        rainbowPaint.setShader(shader);
        RectF rect = new RectF(((int) (x - radius)), ((int) (y - radius)), ((int) (x + radius)), ((int) (y + radius)));
        canvas.drawArc(rect, -90, sweepAngle, false, rainbowPaint);
    }
