package jp.sinya.arcseekbar.demo;
...
public class ShaderSeekArc extends View {
    ...
    private void initSize(int width, int height) {
        ...
        //控件的宽度 = 弧形的宽度 * 弧形跨度的宽度 * 最小尺寸
        mArcWidth = mArcWidthRate * sArcTotalWidth * mMinSize;
        //内环半径 = （1-弧形宽度）* 弧形跨度的宽度 * 最小尺寸
    ...
    private boolean checkTouch(float x, float y) {
        ...
        double distance = Math.sqrt((x - mCenter) * (x - mCenter)
                + (y - mCenter) * (y - mCenter));
        boolean isMoveDistance = distance >= mInnerRadius && distance <= (mInnerRadius + mArcRadius);
        boolean isClick = (Math.abs(mDownX - x) < 50) && (Math.abs(mDownY - y) < 50);
        return isMoveDistance && !isClick;
    }
