...
public class NewCustomSeekBar extends View {
    ...
    //Move事件是否响应
    private boolean mMoveResponse = true;
    //缺口的角度
    ...
    public NewCustomSeekBar(Context context, int innerCircleRadius, int outerCircleRadius, int outerCircleWidth, int progressTextSize) {
        this(context, null);
        initViewData(innerCircleRadius, outerCircleRadius, outerCircleWidth, progressTextSize);
    }
    ...
    private void initViewData(int mInnerCircleRadius, int outerCircleRadius, int outerCircleWidth, int progressTextSize) {
        this.mInnerCircleRadius = mInnerCircleRadius;
        this.mOuterCircleRadius = outerCircleRadius;
        this.mGradientCircleWidth = outerCircleWidth;
        this.mProgressTextSize = progressTextSize;

        mSmallCircleRadius = outerCircleRadius;
        mCenterX = outerCircleRadius + outerCircleWidth;
        mCenterY = outerCircleRadius + outerCircleWidth;
    ...
    private void initData(Context context, AttributeSet attrs) {
        ...
        // 获取自定义组件的属性
        TypedArray types = context.obtainStyledAttributes(attrs, R.styleable.NewCustomSeekBar_csSrcBtn);
        try {
            mInnerCircleRadius = types.getDimensionPixelOffset(R.styleable.NewCustomSeekBar_csSrcBtn_InnerCircle_radius, 150);
            mOuterCircleRadius = types.getDimensionPixelOffset(R.styleable.NewCustomSeekBar_csSrcBtn_ExCircle_radius, 200);
            mGradientCircleWidth = types.getDimensionPixelOffset(R.styleable.NewCustomSeekBar_csSrcBtn_SmartCircle_radius, 20);
            mProgressTextSize = types.getDimensionPixelOffset(R.styleable.NewCustomSeekBar_csSrcBtn_TextSize, 20);
        } finally {
            types.recycle(); // TypeArray用完需要recycle
        }

        mSmallCircleRadius = mOuterCircleRadius;
        mCenterX = mOuterCircleRadius + mGradientCircleWidth;
        mCenterY = mOuterCircleRadius + mGradientCircleWidth;
    }
    ...
    private void drawGradientExCircle(Canvas canvas) {
        ...
        RectF rect = new RectF(mCenterX - mOuterCircleRadius, mCenterY - mOuterCircleRadius, mCenterX + mOuterCircleRadius, mCenterY + mOuterCircleRadius);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        float x = event.getX();
        float y = event.getY();
        float currDegrees = (float) ((float) ((Math.toDegrees(Math.atan2(x - mCenterX, mCenterY - y)) + 360.0)) % 360.0);
        if (currDegrees > (360 - mCurrAngleGap) || currDegrees < mCurrAngleGap) {
            mMoveResponse = false;
            return false;
        }
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if ((Math.pow((x - mCenterX), 2) + (Math.pow(((y - mCenterY)), 2)) > Math.pow((mOuterCircleRadius + mGradientCircleWidth), 2))) {
                    return false;
                }
                mMoveResponse = true;
                break;
            ...
            case MotionEvent.ACTION_UP:
                ...
                double degrees = Math.toDegrees(Math.atan2(x - mCenterX, mCenterY - y));
                if (degrees < 0) {
                    degrees += mCurrAngleGap;
                } else if (degrees > 0) {
                    degrees -= mCurrAngleGap;
                }
                //计算角度
                mDegrees = ((float) ((degrees + (360 - mCurrAngleGap * 2))) % (360 - mCurrAngleGap * 2));
                ...
                mThumbColor = UIColor.interpolate(mStartColor, mEndColor, (mDegrees / (360 - mCurrAngleGap * 2)));
    ...
    public void setCurrProgress(float currProgress) {
        ...
        mThumbColor = UIColor.interpolate(mStartColor, mEndColor, 1 - currProgress);
