...
import jp.sinya.swipeback.demo.library3.core.exception.AfterSaveStateTransactionWarning;
import jp.sinya.swipeback.demo.library3.core.helper.internal.ResultRecord;
import jp.sinya.swipeback.demo.library3.core.helper.internal.TransactionRecord;
import jp.sinya.swipeback.demo.library3.core.queue.Action;
import jp.sinya.swipeback.demo.library3.core.queue.ActionQueue;
...
public class TransactionDelegate {
    ...
    public static final int DEFAULT_POP_TO_ANIM = Integer.MAX_VALUE;
    ...
    /**
     * 继承自{@link ISupportActivity}的Activity,必须是父节点继承自FragmentActivity
     */
    ...
    public TransactionDelegate(ISupportActivity support) {
     ...
     */
    public void loadRootTransaction(final FragmentManager fm, final int containerId, final ISupportFragment to, final boolean addToBackStack, final boolean allowAnimation) {
    ...
    /**
     * 加载多个RootFragment
     * 实际上所有的Fragment都已经创建了，只是根据当前显示哪个，而对应的隐藏掉其他
     *
     * @param fm
     * @param containerId
     * @param showPosition
     * @param tos
     */
    public void loadMultipleRootTransaction(final FragmentManager fm, final int containerId, final int showPosition, final ISupportFragment... tos) {
        enqueue(fm, new Action(Action.ACTION_LOAD) {
            @Override
            public void run() {
    ...
    /**
     * 用于开启新的 非RootFragment
     * Dispatch the start transaction.
     */
    public void dispatchStartTransaction(final FragmentManager fm, final ISupportFragment from, final ISupportFragment to, //
                                         final int requestCode, final int launchMode, final int type) {
        enqueue(fm, new Action(launchMode == ISupportFragment.SINGLE_TASK ? Action.ACTION_POP_MOCK : Action.ACTION_NORMAL) {
            @Override
            public void run() {
    ...
    /**
     * 从当前Fragment切换至目标Fragment，然后pop当前
     * Start the target Fragment and pop itself
     */
    public void startWithPop(final FragmentManager fm, final ISupportFragment from, final ISupportFragment to) {
    ...
    /**
     * 弹栈到指定的Fragment，并将其上层的栈内fragments挤出
     *
     * @param fm
     * @param from
     * @param to
     ...
     */
    public void startWithPopTo(final FragmentManager fm, final ISupportFragment from, final ISupportFragment to, final String fragmentTag, final boolean includeTargetFragment) {
        enqueue(fm, new Action(Action.ACTION_POP_MOCK) {
            @Override
            public void run() {
                int flag = 0;

                //targetFragment是否存在栈内
                ...
                //如果它本身就是最上层fragment，就没有需要挤出去的其它栈内fragments
    ...
    /**
     * 如果想在出栈后, 立刻进行FragmentTransaction操作
     ...
     */
    public void popTo(final String targetFragmentTag, final boolean includeTargetFragment, //
                      final Runnable afterPopTransactionRunnable, final FragmentManager fm, final int popAnim) {
        enqueue(fm, new Action(Action.ACTION_POP_MOCK) {
            @Override
            public void run() {
     ...
     * @param fm
     * @param fragment
     * @param showPreFragment
     */
    public void remove(final FragmentManager fm, final Fragment fragment, final boolean showPreFragment) {
        enqueue(fm, new Action(Action.ACTION_POP, fm) {
            @Override
            public void run() {
    ...
    /**
     * 在这里处理所有 replace/hide/show
     *
     * @param fm
     * @param from
     * @param to
     * @param toFragmentTag
     * @param dontAddToBackStack    true:不加入fragment stack
     * @param sharedElementList
     * @param allowRootFragmentAnim
     * @param type
     */
    private void start(FragmentManager fm, final ISupportFragment from, ISupportFragment to, //
                       String toFragmentTag, boolean dontAddToBackStack, //
                       ArrayList<TransactionRecord.SharedElement> sharedElementList, //
                       boolean allowRootFragmentAnim, int type) {
        ...
        //如果没有启动Fragment，直接加载目标Fragment,并使用replace
        if (from == null) {
    ...
    /**
     * 弹栈到targetFragment，并挤出前面的fragments
     *
     * @param from
     * @param targetFragmentTag
     * @param fm
    ...
    private void mockPopToAnim(Fragment from, String targetFragmentTag, FragmentManager fm, int flag, List<Fragment> willPopFragments, int popAnim) {
        ...
        //remove移除掉targetFragment顶上的栈内fragments,
        //然后根据fromFragment的布局文件copy生成和一个界面再次装载在container中，由这个view来执行结束动画，避免FrameLayout布局container本身被移动
        safePopTo(targetFragmentTag, fm, flag, willPopFragments);
    ...
    /**
     * 重新onStart一个Fragment的时候
     * 对移除的fromFragment其设置Pop退出动画
     *
     * @param from
     * @param to
     * @param exitAnim
    ...
    private void mockStartWithPopAnim(final ISupportFragment from, ISupportFragment to, final Animation exitAnim) {
        ...
        //如果需要替换，会在targetFragment enter进场的时候会在 Fragment的Override函数onCreateAnimation()中执行如下监听
