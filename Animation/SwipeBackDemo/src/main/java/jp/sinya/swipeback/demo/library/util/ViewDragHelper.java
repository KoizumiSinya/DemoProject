 ...
 */
package jp.sinya.swipeback.demo.library.util;
...
public class ViewDragHelper {
    ...
    public boolean settleCapturedViewAt(int finalLeft, int finalTop) {
        ...
        return forceSettleCapturedViewAt(finalLeft, finalTop, (int) VelocityTrackerCompat.getXVelocity(mVelocityTracker, mActivePointerId), (int) VelocityTrackerCompat.getYVelocity
    ...
    public void flingCapturedView(int minLeft, int minTop, int maxLeft, int maxTop) {
        ...
        mScroller.fling(mCapturedView.getLeft(), mCapturedView.getTop(), (int) VelocityTrackerCompat.getXVelocity(mVelocityTracker, mActivePointerId), (int) VelocityTrackerCompat.getYVelocity
                (mVelocityTracker, mActivePointerId), minLeft, maxLeft, minTop, maxTop);
        setDragState(STATE_SETTLING);
    }
    ...
    protected boolean canScroll(View v, boolean checkV, int dx, int dy, int x, int y) {
        if (v instanceof ViewGroup) {
            ...
            for (int i = count - 1; i >= 0; i--) {
                ...
                if (x + scrollX >= child.getLeft() && x + scrollX < child.getRight() && y + scrollY >= child.getTop() && y + scrollY < child.getBottom() && canScroll(child, true, dx, dy, x +
    ...
    private boolean isValidPointerForActionMove(int pointerId) {
        if (!isPointerDown(pointerId)) {
            Log.e(TAG, "Ignoring pointerId=" + pointerId + " because ACTION_DOWN was not received " + "for this pointer before ACTION_MOVE. It likely happened because " + " ViewDragHelper did not " +
                    "receive all the events in the event stream.");
            return false;
        }
        return true;
    }
